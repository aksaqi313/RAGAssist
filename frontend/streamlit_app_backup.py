...
# ============================================================
# STREAMLIT CLOUD + RAG CONFIGURATION
# ============================================================
...
st.set_page_config(
    page_title="Mini-RAG | AI Document Knowledge Assistant",
    page_icon="🌌",
    layout="wide",
...
# ============================================================
# RAG FUNCTIONS — DIRECT STREAMLIT INTEGRATION
# ============================================================

def get_documents():
    """Get all documents directly from the local RAG knowledge base."""
    if not RAG_READY:
        return [], RAG_ERROR
    try:
...
def delete_document(document_id):
    """Delete a document directly from the local RAG knowledge base."""
    if not RAG_READY:
...
def backend_health():
    """Check whether the local RAG system is ready."""
    return RAG_READY


def ask_question(question_text):
    """Generate an answer directly through the local RAG pipeline."""
    if not RAG_READY:
        return None, RAG_ERROR
    if not question_text or not question_text.strip():
        return None, "Question cannot be empty."
    try:
        answer = generate_answer(question_text.strip())
        return answer, None
    except Exception as error:
...
with st.sidebar:
    st.markdown(
        '<div class="brand-title">🤖 Mini-RAG</div>'
...
def render_backend_status():
    if backend_health():
        st.markdown(
            ...
            '🟢 RAG System Ready &nbsp;•&nbsp; FAISS + Sentence Transformers + Gemini are active'
            '</div>',
            unsafe_allow_html=True,
        )
    else:
        st.markdown(
            ...
            '🔴 RAG System Not Ready &nbsp;•&nbsp; Check the RAG configuration and Gemini API key'
            '</div>',
            unsafe_allow_html=True,
        )
    ...
    if uploaded_file is not None:
        ...
        if st.button(
            ...
            with st.spinner(
                ...
                try:
                    if not RAG_READY:
                        st.error(f"❌ RAG system is not ready: {RAG_ERROR}")
                    else:
                        ...
                        st.info(
                            ...
                            f"🔎 Vectors: {info.get('vectors', 'N/A')} | "
                            f"📚 Total Documents: "
                            f"{knowledge_base.get('document_count', 'N/A')}"
                        )

                        st.rerun()

                except Exception as error:
                    st.error("❌ Upload and processing failed.")
...
def render_settings():
    st.markdown(
        ...
        '<div class="section-subtitle">'
        'RAG configuration and workspace reset.'
        '</div>',
        unsafe_allow_html=True,
    )
